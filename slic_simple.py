...
def SegmentsLabelProcess(labels):    #重新编号超像素标签，确保连续性
...
class SLIC(object):      #实现超像素分割，生成超像素特征和邻接矩阵
    def __init__(self, fuse, img1, img2, n_segments=1000, compactness=20, max_iter=20, sigma=0,
                 min_size_factor=0.3,max_size_factor=2):
        self.n_segments = n_segments  # 期待的超像素个数
        self.compactness = compactness  # 超像素的紧凑型
        self.max_iter = max_iter  # slic算法的最大迭代次数
        self.min_size_factor = min_size_factor  # 控制超像素大小因子
        self.max_size_factor = max_size_factor
        self.sigma = sigma  # 在超像素生成之前应用于图像的高斯滤波的标准差。
        self.img1 = img1
        self.img2 = img2
...
class SP_SLIC(object):      #整合 LDA 和 SLIC，处理两时相高光谱图像
    ...
    def LDA_Process(self, curr_labels):
        ...
        print("curr_labels:", curr_labels.shape)
